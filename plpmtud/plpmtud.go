...
func (d *Discoverer) nextState(e event) state { //nolint:gocognit
	evt := e.Type
	mtu := e.MTU
...
func (d *Discoverer) acknowledgedProbe() {
	d.probeTimer.Stop()
	d.probeCount = 0
...
func (d *Discoverer) sendProbeResponse(mtu uint) error {
	return d.prober.SendProbeResponse(mtu)
}
...
func (d *Discoverer) OnProbeRequest(mtu uint) {
	if err := d.sendProbeResponse(mtu); err != nil {
		log.Printf("Failed to send probe response: %s", err)
	}
}
